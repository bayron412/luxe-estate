...
export default function LoginForm({ dict }: LoginFormProps) {
  return (
    <div className="font-display bg-[#EEF6F6] min-h-screen flex items-center justify-center p-4 antialiased text-nordic-dark relative overflow-hidden">
      ...
      <main className="w-full max-w-md z-10">
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_-2px_rgba(25,50,47,0.05)] p-8 sm:p-10 border border-white/50 backdrop-blur-sm">
          <div className="space-y-4">
            {/* Google - Server Action via form submit */}
            <form action={signInWithGoogle}>
              <button 
                type="submit"
                className="group w-full flex items-center justify-center gap-3 bg-white border border-gray-100 rounded-lg p-3.5 text-nordic-dark font-medium transition-all duration-300 hover:shadow-[0_10px_25px_-5px_rgba(25,50,47,0.1)] hover:-translate-y-0.5 relative overflow-hidden"
              >
                ...
                <span className="relative z-10">{dict.google}</span>
              </button>
            </form>

            {/* GitHub - Server Action via form submit */}
            <form action={signInWithGitHub}>
              ...
              >
                ...
                <span className="relative z-10">{dict.github}</span>
              </button>
            </form>
          </div>
          <p className="mt-8 text-center text-sm text-nordic-dark/70">
            {dict.noAccount}{" "}
            <a className="font-semibold text-mosque hover:opacity-80 transition-colors" href="#">
              {dict.signup}
            </a>
          </p>
        </div>
